...
rqdvf = 0.005    #required volume fraction
...
vf = 0
ipd = 3 * r0
noRadPat = int((2*m.pi*(plateInnerR + r0)/ipd))
d1 = plateInnerR+2*r0
theta = np.linspace(0, 2*m.pi, noRadPat)

#creating list of coordinate

layer = 0 #d1 + layer*ipd

while vf<rqdvf:
    for i in theta:
        xp = (d1+ layer*ipd)*m.cos(i)
        yp = (d1 + layer*ipd)*m.sin(i)
        totalVolM += VolM
        plateVol = plateVol-totalVolM
