...
s.CircleByCenterPerimeter(center=(0.0, 0.0), point1=(plateOR, 0.0))
s.CircleByCenterPerimeter(center=(0.0, 0.0), point1=(plateIR, 0.0))
p = mdb.models['Model-1'].Part(name='Plate', dimensionality=THREE_D, type=DEFORMABLE_BODY)
p = mdb.models['Model-1'].parts['Plate']
p.BaseSolidExtrude(sketch=s, depth=plateTh)
s.unsetPrimaryObject()
del mdb.models['Model-1'].sketches['__profile__']
e = p.edges
p.Round(radius=filletR, edgeList=(e.findAt((plateIR,0.0,0.0),),))
...
t = p.MakeSketchTransform(sketchPlane=f.findAt((plateOR/2,0,plateTh),), sketchUpEdge=e.findAt((plateIR,0,plateTh),), sketchPlaneSide=SIDE1, origin=(0.0, 0.0, plateTh))
...
s1.CircleByCenterPerimeter(center=(0.0, 0.0), point1=(clampR, 0.0))
pickedFaces = f.findAt((plateOR/2,0,plateTh),)
e1 = p.edges.findAt((plateIR,0.0,plateTh),)
...
c = p.cells
pickedCells = c.getByBoundingBox(-plateOR,-plateOR,-plateTh,plateOR,plateOR,2*plateTh)#getSequenceFromMask(mask=('[#1 ]', ), )
pickedEdges =(e.findAt((clampR,0,plateTh),), )
p.PartitionCellByExtrudeEdge(line=d1[4], cells=pickedCells, edges=pickedEdges, sense=REVERSE) #source of error 1 wrong sweep axis
...
s.Line(point1=(0.0, 0.0), point2=(coneR, -coneH))
s.FixedConstraint(entity=v.findAt((0.0,0.0),))
s.AngularDimension(line1=g.findAt((coneR,-coneH),), line2=g.findAt((0.0,1.0),), textPoint=(5, -15), value=ConeAngle)
#g[3],g[2]
...
p = mdb.models['Model-1'].parts['Plate']
c = p.cells
cells = c.getByBoundingBox(-plateOR,-plateOR,-plateTh,plateOR,plateOR,2*plateTh)#SequenceFromMask(mask=('[#3 ]',), )
...
side1Faces1 = s.findAt(((plateIR,0,plateTh/2),)) + s.findAt(((3*clampR/4,0.0,0.0),)) + s.findAt(((plateIR+0.2928932*filletR,0.0,0.2928932*filletR),))#getSequenceFromMask(mask=('[#38 ]',), )
...
faces1 = f1.findAt((((plateOR+clampR)/2,0,plateTh),))+f1.findAt((((plateOR+clampR)/2,0,0),)) #getSequenceFromMask(mask=('[#6 ]',), )
...
mdb.models['Model-1'].DisplacementBC(name='BC-2', createStepName='Step-1',
									 region=region, u1=0.0, u2=0.0, u3=coneH, ur1=0.0, ur2=0.0, ur3=0.0,
